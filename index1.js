...
app.post("/upload", upload.single("file"), async (req, res) => {
  try {
    ...
    const result = await db`
      INSERT INTO files (filename, pathname, description)
      VALUES (${req.file.originalname}, ${blob.url}, ${description})
      RETURNING *;
    `;
...
// 📌 Download (just redirect to blob url)
app.get("/files/:id/download", async (req, res) => {
  try {
    const { id } = req.params;
    const result = await db`SELECT * FROM files WHERE id = ${id};`;
    ...
    const file = result[0];
    res.redirect(file.pathname); // blob URL
  } catch (err) {
    res.status(500).json({ message: "Failed to download file", error: err.message });
  }
});
...
app.delete("/files/:id", async (req, res) => {
  try {
    ...
    await del(new URL(file.pathname).pathname);
